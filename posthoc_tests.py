...
def set_posthoc_params(x_ICs, r_ICs, dmg_x_ICs=None, dmg_r_ICs=None):
    ...
    if dmg_x_ICs is not None:
        ph_params['ICs'] = {'r': r_ICs, 'x': x_ICs, 'dmg_r': dmg_r_ICs, 'dmg_x': dmg_x_ICs}
    else:
...
def synet_fw_from_multi_ICs(params, dmg_params, ph_params, digits_rep, labels, trial, ic_index, ICtype='response', ic_zero=True):
    # forward simulate network from multiple ICs without giving network any input
    # ICs can be either during delay or end of each trial
    # ICtype='delay1' or 'delay2' or 'response', trial --> trial type eg (0,1) , ic_index--> IC type eg 0
    ...
    initial_states = ph_params['ICs']['x'] #around fixed point -- shape  num_ICs X N
    initial_states = initial_states.reshape([initial_states.shape[0], initial_states.shape[1]])
    dmg_initial_states = ph_params['ICs']['dmg_x'] #around fixed point -- shape  num_ICs X N
    dmg_initial_states = dmg_initial_states.reshape([dmg_initial_states.shape[0], dmg_initial_states.shape[1]])
    ...
    if ic_zero == False:
        ...
        initial_states = np.vstack((np.zeros([1, net_prs['N']]), initial_states))  # shape = (5, 1000)
        dmg_initial_states = np.vstack((np.zeros([1, dmg_net_prs['N']]), dmg_initial_states))  # shape = (5, 1000)
    ...
    ICs = np.zeros([1, net_prs['N']])
    dmg_ICs = np.zeros([1, dmg_net_prs['N']])
    for i in range(initial_states.shape[0]):

        IC_mat = np.zeros([ph_params['n_ICs'] // initial_states.shape[0], net_prs['N']])
        dmg_IC_mat = np.zeros([ph_params['n_ICs'] // dmg_initial_states.shape[0], dmg_net_prs['N']])
        for j in range(ph_params['n_ICs']//initial_states.shape[0]):
            noise = rng.randn(1, initial_states.shape[1]) * ph_params['x_noise_var']
            dmg_noise = rng.randn(1, dmg_initial_states.shape[1]) * ph_params['x_noise_var']
            IC_mat[j, :] = initial_states[i,:] + noise
            dmg_IC_mat[j, :] = dmg_initial_states[i,:] + dmg_noise

        ICs = np.concatenate((ICs, IC_mat), axis=0)
        dmg_ICs = np.concatenate((dmg_ICs, dmg_IC_mat), axis=0)
    ...
    trajectories = np.zeros([fw_steps, dmg_net_prs['N'], ph_params['n_ICs']])
    z_mat, zd_mat = np.zeros([fw_steps, dmg_net_prs['d_output'], ph_params['n_ICs']]), np.zeros([fw_steps, dmg_net_prs['d_input']-1, ph_params['n_ICs']])
    # fw simulate from set of initial conditions for i_params['n_fw'] steps
    ...
    for k in range(ICs.shape[0]):
        ...
        dmg_x_mat, dmg_r_mat = np.zeros([dmg_net_prs['N'], fw_steps]), np.zeros([dmg_net_prs['N'], fw_steps])
        z_, zd_ = np.zeros([dmg_net_prs['d_output'], fw_steps]), np.zeros([dmg_net_prs['d_input']-1, fw_steps])
        ...
        for i in range(fw_steps):
            ...
            r = np.tanh(x)
            u = np.matmul(wo.T, r)
            dmg_input = np.concatenate((u,np.zeros([2,])),axis=None)
...
def attractor_type(params, ph_params, digits_rep, labels, synet=False, dmg_params=None):
    ...
    if synet:
        trajectories, z_mat, zd_mat = synet_fw_from_multi_ICs(params, dmg_params, ph_params, digits_rep, labels, (0,0), 0, ICtype='response', ic_zero=True)
    else:
